    ...
    const plugin = {
        ...
        async load(file: string) {
            ...
            const fromMap = (fileName: string) => {
                return fsMap.get(fileName) || fsMap.get(fileName + '.tsx') || fsMap.get(fileName + '.ts') || fsMap.get(fileName  + '.js  ');
            };
            ...
            host.fileExists = (fileName: string) => {
                const exists =  fromMap(fileName) != null || oFileExists.call(this, fileName);
                return exists;
            };
...
`);
                ...
                const transpiledCode = transpiledMap.get(
                    block.filename.replace(".ts", ".js"),
                ) ?? transpiledMap.get(block.filename.replace(".tsx", ".js"));
                const sourceMapContent = transpiledMap.get(
                    block.filename.replace(".ts", ".js.map"),
        ...
        `);
            ...
            const filteredFiles = new Map(
                [...transpiledMap]
                    .filter(([k, v]) => files.has(k.replace(".js", ".ts")) || files.has(k.replace(".js", ".tsx")))
